# JSON was made using tiled, Im too lazy to make my own editor just use whatever there is to quickly create room I'm ok with that
...
with open(file_path, 'r') as file:
    ...
    data = json.load(file)
    # TODO: Next time set the grid width and height from here
    # tileheight = data['tileheight']
    # width = data['width']
    # height = data['height']

# Prepare pre-rendered background
pre_rendered_bg: pygame.Surface = pygame.Surface((GRID_WIDTH * CELL_SIZE, GRID_HEIGHT * CELL_SIZE))
...
world_map_grid_data = "".join(
    "1" if data['layers'][0]['data'][y * GRID_WIDTH + x] != 0 else "0"
...
for layer in data['layers']:
    for index, tile_id in enumerate(layer['data']):
        ...
        src_x = (tile_id % 32) * CELL_SIZE  # Assuming spritesheet has 32 tiles per row
        src_y = (tile_id // 32) * CELL_SIZE
        pre_rendered_bg.blit(spritesheet, (dest_x, dest_y), (src_x, src_y, CELL_SIZE, CELL_SIZE))
...
class Player:
    def __init__(self, x, y):
        self.surf: pygame.Surface = pygame.Surface((CELL_SIZE, CELL_SIZE))
        self.surf.fill("red")
...
def main():
    ...
    while running:
        ...
        screen.blit(pre_rendered_bg, (0, 0))  # Draw the pre-rendered grid world map
        ...
        pygame.display.set_caption(f"FPS: {clock.get_fps():.2f} | Iteration Time: {dt}ms")
